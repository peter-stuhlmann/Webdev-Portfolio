...
export default function CookieConsentBanner() {
  const { content, cookieOptIn, optIn, optOut } = useContext(Context);

  return cookieOptIn === undefined ? (
    <CookieConsent>
      <p>
        {content.cookieConsent.text}
        <Link to={content.cookieConsent.link[1]}>
          {content.cookieConsent.link[0]}
        </Link>
        .
      </p>
      <button type="button" className="decline" onClick={() => optOut()}>
        {content.cookieConsent.buttons.decline}
      </button>
      <button type="button" className="accept" onClick={() => optIn()}>
        {content.cookieConsent.buttons.accept}
      </button>
    </CookieConsent>
  ) : null;
}
